    ...
    <main className="min-h-screen bg-white">
      ...
      {breakingPosts.length > 0 && (
        <div className="border-b border-gray-200">
          <div className="container mx-auto px-4 flex items-center h-12">
            ...
            <div className="flex-grow overflow-hidden relative h-full flex items-center pl-4 bg-white">
              <div className="animate-marquee whitespace-nowrap">
                {breakingPosts.map((post) => (
                  <span key={post.id} className="mr-12 text-sm font-medium text-gray-800">
            ...
            <div className="flex-shrink-0 flex gap-1 pl-4">
              <button className="w-6 h-6 border border-gray-300 flex items-center justify-center text-gray-500 hover:bg-gray-100 text-xs">‹</button>
              <button className="w-6 h-6 border border-gray-300 flex items-center justify-center text-gray-500 hover:bg-gray-100 text-xs">›</button>
            </div>
          </div>
        </div>
      ...
      <section className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-1">
          ...
          {mainHeroPost ? (
            <div className="lg:col-span-2 relative h-[500px] group overflow-hidden bg-gray-100">
            ...
            <div className="lg:col-span-2 h-[500px] bg-gray-100 flex items-center justify-center text-gray-400">
          ...
          <div className="flex flex-col gap-1 h-[500px]">
            {sideHeroPosts.map((post) => (
              <div key={post.id} className="relative flex-1 group overflow-hidden bg-gray-100">
            ...
            {[...Array(2 - sideHeroPosts.length)].map((_, i) => (
              <div key={`empty-${i}`} className="flex-1 bg-gray-50 flex items-center justify-center text-xs text-gray-400">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:w-3/4">
            ...
            <div className="mb-12">
              <div className="flex items-center justify-between mb-6 border-b border-gray-200 pb-2">
                <h3 className="text-xl font-black uppercase tracking-tight">Featured News</h3>
                <Link href="/news" className="text-xs font-bold text-gray-500 hover:text-blue-600 uppercase">View All</Link>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {featuredPosts.map((post) => (
                  <article key={post.id} className="flex flex-col group">
                    <div className="relative h-60 overflow-hidden mb-4 rounded-sm bg-gray-100">
                    ...
                    <div>
                      <h3 className="text-xl font-bold mb-2 leading-tight group-hover:text-blue-600 transition">
            ...
            <div>
              <div className="flex items-center justify-between mb-6 border-b border-gray-200 pb-2">
                <h3 className="text-xl font-black uppercase tracking-tight">Latest Stories</h3>
              </div>
              <div className="flex flex-col gap-8">
                {latestPosts.map((post) => (
                  <article key={post.id} className="flex gap-6 group border-b border-gray-100 pb-8 last:border-0">
                    <div className="w-1/3 relative h-32 md:h-40 flex-shrink-0 overflow-hidden rounded-sm bg-gray-100">
                      <Link href={`/news/${post.slug}`}>
                        <Image
                          src={getImageUrl(post.featuredImage)}
                          alt={post.title}
                    ...
                    <div className="flex-1 flex flex-col justify-center">
                      ...
                      <h3 className="text-lg md:text-xl font-bold mb-2 leading-tight group-hover:text-blue-600 transition">
                        <Link href={`/news/${post.slug}`}>{post.title}</Link>
                      </h3>
                      <p className="text-gray-500 text-sm line-clamp-2 hidden md:block mb-2">{post.excerpt}</p>
          ...
          <div className="lg:w-1/4">
            ...
            <div className="bg-gray-50 p-6 mb-8 text-center">
              <h4 className="font-bold uppercase tracking-wider mb-4">Follow Us</h4>
              <div className="flex justify-center gap-2">
                {['fb', 'tw', 'in', 'yt'].map(s => (
                  <div key={s} className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 hover:bg-black hover:text-white transition cursor-pointer">
                    {s}
                  </div>
                ))}
              </div>
